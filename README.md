...
## 1. 프로젝트 개요 <a name="project outline"></a>
 ...
 - 프로젝트를 진행하기에 앞서 팀원들과 건물별 모델, 건물유형별 모델 중 어떠한 방식으로 진행할 것인지 논의하였습니다.
   건물별 모델을 구축한다면, train 데이터가 각 모델별로 2040개 밖에 되지 않아, 모델의 성능을 장담할 수 없을 뿐만 아니라
   주관사인 한국에너지공단에서 새로운 건물데이터 추가될 시 예측이 어려워지는 점을 고려했습니다.
   따라서 건물유형별 모델을 우선적으로 만들어 보기로 했습니다.
